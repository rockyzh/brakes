...
class Circuit extends EventEmitter {
  ...
  _execPromise() {
    return new Promise((resolve, reject) => {
      // start timeout timer
      let execTimeout = false;
      const execGeneration = this._brakes._circuitGeneration;
      const startTime = Date.now();
      const timeoutTimer = setTimeout(() => {
        if (!(this._opts.notBrakeTimeout || this._brakes._opts.notBrakeTimeout)) {
      ...
      this._serviceCall.apply(this._this, arguments).then(result => {
        ...
        if (execTimeout) {
          const endTime = Date.now() - startTime;
          this._brakes.emit('timeout', endTime, new TimeOutError(consts.TIMEOUT), execGeneration);
        }
        ...
        if (execTimeout) {
          // 优先发timeout
          const endTime = Date.now() - startTime;
          const err = new TimeOutError(consts.TIMEOUT);
          this._brakes.emit('timeout', endTime, err, execGeneration);
        }
